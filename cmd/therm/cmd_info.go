...
// NewInfoCmd creates a Command with sane defaults
func NewInfoCmd(ui clui.UI) clui.Command {
	c := &infoCmd{
		ui:      ui,
		flagset: flagset.NewFlagSet("info", flag.ExitOnError),
	}
	c.init()
	return c
}
...
Description:
  ...
  Show server information as JSON or YAML.

Example:

  therm info
  therm info --format=json
`
}
...
// behavioral changes.
func (c *infoCmd) Run() clui.ExitCode {
	// Logging.
	...
	exec.Block(g)
	{
		g.Add(func() error {
			...
			content, err := outputContent(outputFormat, result)
			if err != nil {
				return errors.WithStack(err)
			}
