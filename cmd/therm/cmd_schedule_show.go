...
// NewScheduleShowCmd creates a Command with sane defaults
func NewScheduleShowCmd(ui clui.UI) clui.Command {
	c := &scheduleShowCmd{
		ui:      ui,
...
func (c *scheduleShowCmd) Help() string {
	return `
Usage: 

  schedule [flags] show <name>
...
Example:
  ...
  therm schedules show ac025ec4
  therm schedules show ac025ec4 --foramt=json
`
}

// Synopsis should return a one-line, short synopsis of the command.
// This should be short (50 characters of less ideally).
func (c *scheduleShowCmd) Synopsis() string {
	return "Show a schedule."
}
...
// behavioral changes.
func (c *scheduleShowCmd) Run() clui.ExitCode {
	// Logging.
	...
	{
		g.Add(func() error {
			response, _, err := client.Query("GET", fmt.Sprintf("/1.0/schedules/%s", id), nil, "")
			if err != nil {
